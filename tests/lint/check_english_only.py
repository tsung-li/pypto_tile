...
DEFAULT_EXCLUDED_PATTERNS = ["3rdparty"]
...
def main():
    ...
    for f in all_files:
        ...
        if not any(
            str(relative_path).startswith(pattern) for pattern in excluded_patterns
        ):
    ...
    for file_path in files_to_check:
        ...
        if is_english_only:
            ...
            for line_num, non_english_text in violations[:5]:
                print(
                    f"✗ {file_path.relative_to(root_path)}:{line_num} {non_english_text}"
                )
            if len(violations) > 5:
                print(
                    f"  ... and {len(violations) - 5} more line(s) in {file_path.relative_to(root_path)}"
                )
